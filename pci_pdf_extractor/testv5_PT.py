#!/usr/bin/env python3
"""
Extrator Automático de Requisitos PCI DSS (Versão 5 - Melhorada) - Versão Portuguesa
Automatic PCI DSS Requirements Extractor (Version 5 - Improved) - Portuguese Version
Extrai todos os requisitos do documento SAQ D v4.0.1
Formato de saída: {'req_num': '...', 'text': '...', 'tests': [...], 'guidance': '...'}
"""
import re
import json
import PyPDF2
...
class PCIRequirementsExtractor:
    """Classe principal para extrair requisitos PCI DSS de um PDF"""
            ...
            with open(self.pdf_path, 'rb') as file:
                ...
                for page_num in range(len(pdf_reader.pages)):
                    ...
                    if re.search(r'^1\.1\.1\s+', page_text, re.MULTILINE):
                        print(f"Página inicial detectada: {page_num + 1} (contém 1.1.1)")
                        return page_num
                        
        except Exception as e:
            print(f"Erro ao procurar página inicial: {e}")
            
        # Fallback para página 16 se não encontrado
        print("Página inicial não encontrada, usando página 16 como padrão")
        return 15
    ...
    def clean_text(self, text: str) -> str:
        """Limpa texto extraído do PDF removendo artefatos portugueses"""
        # Remove artefatos abrangentes de copyright e página
        text = re.sub(r'PCI DSS v[\d.]+\s+SAQ D para Comerciantes.*?Outubro de \d+', '', text, flags=re.DOTALL | re.IGNORECASE)
        text = re.sub(r'SAQ D do PCI DSS v[\d.]+.*?Página \d+.*?(?:No Local|Não no Local)', '', text, flags=re.DOTALL | re.IGNORECASE)
        ...
        text = re.sub(r'PCI DSS v[\d.]+.*?Questionário de Autoavaliação.*?(?=\n)', '', text, flags=re.IGNORECASE)
        text = re.sub(r'Seção\s+\d+\s*:\s*Questionário de Autoavaliação.*?(?=\n)', '', text, flags=re.IGNORECASE)
        
        # Limpa referências de página isoladas
        text = re.sub(r'Página \d+\s*$', '', text, flags=re.MULTILINE | re.IGNORECASE)
        
        # Substitui múltiplas quebras de linha por únicas
        text = re.sub(r'\n\s*\n', '\n\n', text)
    ...
    def is_test_line(self, line: str) -> bool:
        """Verifica se uma linha é uma linha de teste"""
...
def main():
    pdf_path = "/Users/thomasmionnet/Downloads/PCI-DSS-v4-0-1-SAQ-D-Merchant-PT.pdf"
    print("EXTRATOR PCI DSS PORTUGUÊS")
    print("=" * 60)
    ...
    if requirements:
        ...
        extractor.save_to_json("pci_requirements_v5_PT.json")  # Mantenha JSON para compatibilidade também
    else:
