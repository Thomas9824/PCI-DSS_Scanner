#!/usr/bin/env python3
"""
Extracteur automatique d'exigences PCI DSS (Version 5 - Améliorée) - Version Portugaise
Automatic PCI DSS Requirements Extractor (Version 5 - Improved) - Portuguese Version
Extrait toutes les exigences du document SAQ D v4.0.1 portugais
...
class PCIRequirementsExtractor:
    """Classe principale pour extraire les exigences PCI DSS depuis un PDF portugais"""

    def __init__(self, pdf_path: str):
        self.pdf_path = pdf_path
        self.requirements = []
        
        # Marqueurs pour identifier les sections en portugais
        self.test_indicators = ['• Examine', '• Observe', '• Entreviste', '• Revise']
    ...
    def clean_text(self, text: str) -> str:
        """Nettoie le texte extrait du PDF en supprimant les artefacts portugais"""
        # Remove comprehensive copyright and page artifacts
        text = re.sub(r'PCI DSS v[\d.]+\s+SAQ D para Comerciantes.*?Outubro de \d+', '', text, flags=re.DOTALL | re.IGNORECASE)
        text = re.sub(r'SAQ D do PCI DSS v[\d.]+.*?Página \d+.*?(?:Implementado|Não Implementado)', '', text, flags=re.DOTALL | re.IGNORECASE)
        text = re.sub(r'© 2006[−-]\d+\s+PCI Security Standards Council,\s+LLC\.\s+Todos os Direitos Reservados\.\s+Página\s+\w+', '', text, flags=re.IGNORECASE | re.DOTALL)
        text = re.sub(r'© 2006[−-]\d+.*?PCI Security Standards Council.*?LLC.*?Todos os Direitos Reservados.*?Página \d+', '', text, flags=re.IGNORECASE | re.DOTALL)
        text = re.sub(r'© 2006[−-]\d+.*?LLC.*?Todos os Direitos Reservados\.?', '', text, flags=re.IGNORECASE)
        text = re.sub(r'PCI Security Standards Council.*?LLC.*?Todos os Direitos Reservados.*?Página \d+', '', text, flags=re.IGNORECASE)
        ...
        text = re.sub(r'Seção\s+\d+\s*:\s*Self\s*-\s*Assessment\s+Questionnaire.*?(?=\n)', '', text, flags=re.IGNORECASE)
        
        # Clean isolated Page references
        text = re.sub(r'Página \d+\s*$', '', text, flags=re.MULTILINE | re.IGNORECASE)
    ...
    def is_test_line(self, line: str) -> bool:
        ...
        test_verbs = ['Examine', 'Observe', 'Entreviste', 'Revise', 'Examinar', 'Observar', 'Entrevistar', 'Revisar']
    ...
    def parse_requirements(self, content: str) -> List[Dict[str, Any]]:
        ...
        while i < len(lines):
            ...
            if current_req:
                ...
                # Vérifier si c'est la section Notas de Aplicabilidade 
                elif line.startswith(self.applicability_marker) or line.startswith('Observações de Aplicabilidade'):
                ...
                # Vérifier si c'est la section Orientação
                elif line.startswith(self.guidance_marker) or line.startswith('Orientação'):
    ...
    def _extract_tests_from_text_line_multiline(self, line: str, current_req: Dict[str, Any], all_lines: List[str], current_index: int) -> Tuple[str, int]:
        ...
        for indicator in self.test_indicators:
            verb = indicator[2:]  # Enlever "• " pour avoir juste "Examinar", "Observar", etc.
    ...
    def _clean_test_text(self, text: str) -> str:
        """Nettoie le texte d'un test en supprimant les artefacts portugais"""
        # Remove layout artifacts - more comprehensive patterns
        text = re.sub(r'SAQ D do PCI DSS.*?Página \d+.*', '', text, flags=re.IGNORECASE)
        text = re.sub(r'© 2006[−-]\d+.*?PCI Security Standards Council.*?LLC.*?Todos os Direitos Reservados.*?Página \d+', '', text, flags=re.IGNORECASE | re.DOTALL)
        text = re.sub(r'© 2006[−-]\d+.*?LLC.*?Todos os Direitos Reservados.*', '', text, flags=re.IGNORECASE)
        text = re.sub(r'PCI Security Standards Council.*?LLC.*', '', text, flags=re.IGNORECASE)
        text = re.sub(r'Todos os Direitos Reservados.*?Página \d+', '', text, flags=re.IGNORECASE)
        text = re.sub(r'Página \d+.*', '', text, flags=re.IGNORECASE)
        text = re.sub(r'Implementado.*?Não Implementado', '', text, flags=re.IGNORECASE)
        text = re.sub(r'♦\s*Consulte.*', '', text, flags=re.IGNORECASE)
        
        # Remove response table artifacts
        text = re.sub(r'com CCW Não Aplicável Não Testado Não.*', '', text, flags=re.IGNORECASE)
        text = re.sub(r'Implementado\s+Implementado com CCW\s+Não Aplicável\s+Não Testado\s+Não Implementado', '', text, flags=re.IGNORECASE)
        text = re.sub(r'(Implementado|Não Implementado|Não Aplicável|Não Testado|CCW)(\s+(Implementado|Não Implementado|Não Aplicável|Não Testado|CCW))*', '', text, flags=re.IGNORECASE)
    ...
    def _should_ignore_line(self, line: str) -> bool:
        """Détermine si une ligne doit être ignorée (patterns portugais)"""
        ignore_patterns = [
            r'^SAQ D do PCI DSS',
            r'^© 2006[−-]\d+',
            r'^Página \d+',
            r'^Outubro 2024',
            r'^Requisito do PCI DSS',
        ...
        if re.search(r'© 2006[−-]\d+.*?PCI Security Standards Council.*?LLC.*?Todos os Direitos Reservados', line, re.IGNORECASE):
            return True
    ...
    def _remove_response_artifacts(self, text: str) -> str:
        """Supprime les artefacts de cases de réponse du questionnaire portugais"""
        ...
        patterns_to_remove = [
            ...
            r'(Implementado|Não Implementado|Não Aplicável|Não Testado|CCW)(\s+(Implementado|Não Implementado|Não Aplicável|Não Testado|CCW))+',
            r'♦\s*Consulte.*?(?=\n|$)',
            r'\(Marque uma resposta.*?\)',
            r'© 2006[−-]\d+.*?PCI Security Standards Council.*?LLC.*?Todos os Direitos Reservados.*?Página \d+',
    ...
    def save_to_json(self, output_file: str = "pci_requirements_v5_PT.json"):
    ...
    def save_to_csv(self, output_file: str = "pci_requirements_v5_PT.csv"):
...
def main():
    pdf_path = "PCI-DSS-v4-0-1-SAQ-D-Merchant-PT.pdf"
    print("EXTRACTEUR PCI DSS PORTUGAIS")
    print("=" * 60)
    ...
    if requirements:
        ...
        extractor.save_to_csv("pci_requirements_v5_PT.csv")
        extractor.save_to_json("pci_requirements_v5_PT.json")  # Garde aussi JSON pour compatibilité
    else:
