#!/usr/bin/env python3
"""
Extracteur automatique d'exigences PCI DSS (Version 5 - Améliorée) - Version Espagnole
Automatic PCI DSS Requirements Extractor (Version 5 - Improved) - Spanish Version
Extrait toutes les exigences du document SAQ D v4.0.1 espagnol
...
class PCIRequirementsExtractor:
    """Classe principale pour extraire les exigences PCI DSS depuis un PDF espagnol"""
    ...
    def clean_text(self, text: str) -> str:
        """Nettoie le texte extrait du PDF en supprimant les artefacts espagnols"""
        # Remove comprehensive copyright and page artifacts
        text = re.sub(r'PCI DSS v[\d.]+\s+SAQ D para Comerciantes.*?Octubre de \d+', '', text, flags=re.DOTALL | re.IGNORECASE)
        text = re.sub(r'SAQ D de PCI DSS v[\d.]+.*?Página \d+.*?(?:En Su Lugar|No En Su Lugar)', '', text, flags=re.DOTALL | re.IGNORECASE)
        ...
        text = re.sub(r'Sección\s+\d+\s*:\s*Self\s*-\s*Assessment\s+Questionnaire.*?(?=\n)', '', text, flags=re.IGNORECASE)
    ...
    def parse_requirements(self, content: str) -> List[Dict[str, Any]]:
        ...
        while i < len(lines):
            ...
            if current_req:
                ...
                elif line.startswith(self.applicability_marker) or line.startswith('Notas de Aplicabilidad'):
                ...
                # Vérifier si c'est la section Orientación
                elif line.startswith(self.guidance_marker) or line.startswith('Orientación'):
    ...
    def _clean_test_text(self, text: str) -> str:
        """Nettoie le texte d'un test en supprimant les artefacts espagnols"""
    ...
    def _should_ignore_line(self, line: str) -> bool:
        """Détermine si une ligne doit être ignorée (patterns espagnols)"""
        ignore_patterns = [
            r'^SAQ D de PCI DSS',
            r'^© 2006[−-]\d+',
            r'^Página \d+',
        ...
        if re.search(r'© 2006[−-]\d+.*?PCI Security Standards Council.*?LLC.*?Todos los Derechos Reservados', line, re.IGNORECASE):
            return True
    ...
    def _remove_response_artifacts(self, text: str) -> str:
        """Supprime les artefacts de cases de réponse du questionnaire espagnol"""
        # Remove all variations of response checkboxes and copyright/page artifacts
        patterns_to_remove = [
            r'con CCW No Aplicable No Probado No.*?(?=\n|$)',
    ...
    def save_to_json(self, output_file: str = "pci_requirements_v5_ES.json"):
    ...
    def save_to_csv(self, output_file: str = "pci_requirements_v5_ES.csv"):
...
def main():
    ...
    if requirements:
        ...
        extractor.save_to_csv("pci_requirements_v5_ES.csv")
        extractor.save_to_json("pci_requirements_v5_ES.json")  # Garde aussi JSON pour compatibilité
    else:
