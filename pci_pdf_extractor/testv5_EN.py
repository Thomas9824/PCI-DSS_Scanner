...
class PCIRequirementsExtractor:
    ...
    def find_start_page(self) -> int:
        """Automatically detects the start page (containing 1.1.1)"""
        try:
            with open(self.pdf_path, 'rb') as file:
                ...
                for page_num in range(len(pdf_reader.pages)):
                    ...
                    if re.search(r'^1\.1\.1\s+', page_text, re.MULTILINE):
                        print(f"Start page detected: {page_num + 1} (contains 1.1.1)")
                        return page_num
                        
        except Exception as e:
            print(f"Error searching for start page: {e}")
            
        # Fallback to page 16 if not found
        print("Start page not found, using page 16 as default")
        return 15
    ...
    def clean_text(self, text: str) -> str:
        ...
        text = re.sub(r'PCI Security Standards Council.*?LLC.*?All Rights Reserved.*?Page \d+', '', text, flags=re.IGNORECASE)
        ...
        text = re.sub(r'Section\s+\d+\s*:\s*Self\s*-\s*Assessment\s+Questionnaire.*?(?=\n)', '', text, flags=re.IGNORECASE)
...
def main():
    ...
    if requirements:
        ...
        extractor.save_to_csv("pci_requirements_v5_EN.csv")
        extractor.save_to_json("pci_requirements_v5_EN.json")  # Keep JSON for compatibility too
    else:
